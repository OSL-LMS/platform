...
- **`unstable_cache` serializa a JSON, y eso convierte un `Date` en cadena.** `getBroadcasts` revive `startsAt` al SALIR de la caché. No es defensa preventiva: tumbó la landing entera con `startsAt.getTime is not a function` el 2026-08-01, y ningún test lo vio. Fuera del servidor de Next el import dinámico de `next/cache` falla y el módulo cae al lector directo, que devuelve `Date` de verdad — así que **todo lo que corre bajo Node pelado ejercita la rama que no serializa**, y este repositorio no tiene ningún test que corra dentro del proceso de Next. `curriculum.ts` no lo sufre porque sus nodos no llevan fechas: "replica el patrón de `curriculum.ts`" era cierto y aun así insuficiente. Cualquier módulo futuro que meta un tipo no-JSON tras `unstable_cache` tiene el mismo agujero y la misma ausencia de cobertura.
