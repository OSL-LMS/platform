 ...
 *
 *  Esa exigencia SOLA sí relajaba el control, y por eso no va sola. La regla de
 *  fondo: **este detector tiene que ver lo mismo que verá el parser de URL del
 *  navegador**, y ese parser normaliza antes de mirar. Cada carácter que él
 *  descarta y nosotros no es un bypass, porque este detector es la ÚNICA
 *  puerta: si no casa, ni el control de esquema ni la allowlist de host llegan
 *  a correr. Tres piezas, y hay que repensarlas juntas:
 *
 *  1. `stripUrlNoise` normaliza lo que el parser descarta (ver ahí).
 *  2. `[/\\]{2}` en vez de `//`: para un esquema especial el parser trata `\`
 *     como `/`, así que `/\evil…`, `\\evil…` y `\/evil…` son relativos a
 *     protocolo igual que `//evil…` — el caso que §8.6 nombra por su nombre.
 *  3. `DANGEROUS_SCHEME`, lista cerrada que cae aunque lleve espacio detrás:
 *     `javascript: alert(1)` ejecuta en un `href` y ninguna prosa del temario
 *     empieza por `javascript:`. */
const URL_LIKE = /^(?:[a-z][a-z0-9+.-]*:\S|[/\\]{2})/i;

/**
 * Lo que el parser de URL descarta en silencio:
 * - tab, LF y CR **en cualquier posición** (`https:⟨tab⟩//evil` navega);
 * - todo control C0 y el espacio **al principio**.
 *
 * Lo segundo no lo cubría `\s` de JavaScript, que incluye `\t \n \v \f \r` y el
 * espacio pero **no** U+0000–U+0008 ni U+000E–U+001F. Con `^\s*` como puerta,
 * un `\u0001` inicial hacía que el detector no llegara nunca al esquema — y en
 * JSON `"\u0001https://…"` es legal y tan visible en un diff como lo era el tab.
 */
const stripUrlNoise = (value: string) =>
  value.replace(/[\t\n\r]/g, "").replace(/^[\u0000- ]+/, "");
...
function checkUrlSafety(
  ...
  // Las dos primeras barras se normalizan a `//` para que el rechazo llegue por
  // HOST —que es la información útil— y no por "no se puede parsear".
  const candidate = /^[/\\]{2}/.test(trimmed)
    ? `https://${trimmed.slice(2)}`
    : trimmed;
