...
**Desde PRD-006 viven en dos sitios, y el criterio es qué importan *y qué leen*** — tres escanean el árbol por ruta además de importar, y esa dependencia decide dónde van:

- **En `scripts/` (raíz)**: `check-curriculum.ts`, `check-curriculum-golden.ts`, `check-lessons.ts`, `check-curriculum-identity.ts`, `check-window.ts` y `check-boundaries.ts` (nuevo).
- **En `apps/web/scripts/`**: `check-access-bridge.ts`, `check-analytics.ts`, `check-format-message.ts`, `check-schedule.ts`, `check-tutor-turn.ts`, `check-secrets.ts` — los seis que dependen de `apps/web/src/lib`. Viajaron **junto a** `src/`, así que sus imports no cambiaron ni un carácter; un diff en esa línea es señal de que algo se movió mal.
- **De integración** (exigen Postgres): `check-curriculum-load.ts`.

`pnpm curriculum:check` ejecuta **cuatro** (golden, currículo, lecciones, identidad); el golden va primero porque `node:assert` lanza al primer fallo y un fallo trivial impediría correr el más cargado. **La cadena se partió en PRD-006** porque CI la corre: `check-curriculum-load.ts` exige `CURRICULUM_TEST_DATABASE_URL` y sale 1 sin ella, así que un único encadenado con `&&` la habría dejado roja en cada PR. Vive en `curriculum:check:db`, que corre el operador.

**`check-boundaries.ts` vigila los controles, no el producto**: fronteras entre los tres paquetes en las tres direcciones —incluida `packages/shared` → `apps/`, la peor, porque arrastra código local de una app al proceso de la otra—, CODEOWNERS en los dos sentidos y **con owners**, un solo `export type Access` en el árbol, que ningún Client Component importe `@shared/*` por valor, y que ninguna entrada de `scripts` apunte a un archivo inexistente. Lo de los owners no es celo: CODEOWNERS es *last-match-wins* y una entrada con patrón y sin dueño **retira** la propiedad, así que borrar el `@owner` de una línea —un diff que parece limpieza de espacios— desprotegería el archivo con el check en verde.

**Deriva registrada (PRD-006, re-revisión post-implementación)**: la segunda mitad de la invariante (c) de PRD-002 —`assert.match(route.text, /curriculumSlug\(\)/)` sobre el handler de `/api/chat`— **se retiró**. Llevaba fallando desde el paso E de PRD-005, que vació ese handler a proxy y se llevó la resolución del currículo a `apps/api`; nadie lo vio porque hasta PRD-006 no había nada que ejecutara la comprobación. No se reubicó a propósito: un grep equivalente tendría que escanear `apps/api/src` y eso ensancharía en silencio la lista de raíces que `check-curriculum.ts` fija como constante. La propiedad que protegía —que el cliente no elige currículo— la sostiene ahora la estructura: `turn.dto.ts` con `whitelist` y `forbidNonWhitelisted` aplicados globalmente en `bootstrap.ts` convierten un campo `curriculum` en el cuerpo en un 400. La mitad que sobrevive (`assert.doesNotMatch(/body\.curriculum/)`) sigue cubriendo el proxy.
...
**El repositorio tiene CI desde PRD-006, y es ADVISORIA — no bloquea nada.** `.github/workflows/checks.yml` corre en cada `pull_request` lo que no necesita base de datos: `check-boundaries.ts`, `pnpm curriculum:check`, `check-window.ts`, las seis de `apps/web/scripts/`, `pnpm --filter web build` y la spec `build-boot` de `apps/api` con alcance de archivo. Lo que exige `CURRICULUM_TEST_DATABASE_URL` o `API_TEST_DATABASE_URL` queda fuera y lo corre el operador.

**Que esté en rojo no impide mergear**, porque "Require review from Code Owners" y la protección de rama quedaron declinadas por decisión del propietario (PRD-006 § 3) — igual que CODEOWNERS, esto compra visibilidad, no bloqueo. Y como Railway despliega al mergear (ver arriba, en esta misma sección), un merge con CI roja sale a producción igual. Quien mergea es el gate.

**Cinco invariantes del workflow, no opcionales** (PRD-006 § 8.5), porque el repositorio es público y `CONTRIBUTING.md` manda hacer fork, así que esto ejecuta contenido de terceros: el único disparador es `pull_request` —añadir `pull_request_target`, `workflow_run` o `issue_comment` comparte secretos y token de escritura con código no confiable—; `permissions: contents: read` declarado a nivel de workflow (uno a nivel de job lo sobrescribiría en silencio); cero referencias a `secrets.*`; acciones de terceros ancladas por SHA de commit; y `runs-on` hospedado por GitHub, porque uno autohospedado convierte el residual de minutos de Actions en acceso persistente a una máquina. El residual aceptado —`pnpm install` ejecuta scripts de ciclo de vida— está acotado a gastar minutos **mientras** el ajuste de GitHub "Fork pull request workflows from outside collaborators" siga exigiendo aprobación para quien contribuye por primera vez, que es su defecto en repositorio público.

`checks.yml` usa además `fetch-depth: 0` a propósito: con el clon superficial por defecto, `baseRef()` de `check-curriculum-identity.ts` no resuelve ninguna ref de rama, cae a `HEAD` y compara el currículo consigo mismo — un OK vacío. Ese script falla ahora en vez de degradar cuando `GITHUB_ACTIONS` está puesto, porque `checks.yml` es editable en un PR y la defensa no podía vivir solo ahí.

### Entorno y despliegue

**Dos lecciones del despliegue de PRD-005, aprendidas rompiendo producción el 2026-07-30. Léelas antes de tocar variables.**

- **No uses referencias cruzadas de Railway (`${{servicio.VARIABLE}}`) para un secreto compartido entre servicios.** Parecen la opción limpia —un solo valor, rotación propagada sola— y crean una dependencia invisible: `apps/api` tenía su `ANTHROPIC_API_KEY` apuntando a la de `tutor-app`, el paso E exigía borrarla de `tutor-app`, y al borrarla la referencia quedó resolviendo a cadena vacía. `apps/api` entró en bucle de arranque. Duplicar el valor es más tosco y no se rompe por un borrado en otro servicio.
- **El fallo cerrado de `ANTHROPIC_API_KEY` en `apps/api` arrastra acceso y cobro con él.** Que el tutor no arranque sin su clave es correcto y deliberado; la consecuencia que no estaba escrita es que ese mismo arranque sirve `/v1/access` y el **webhook de Paddle**, así que una clave del tutor ausente deja el muro de pago degradando abierto y los pagos sin registrar hasta que alguien lo note. Paddle reintenta durante días, así que se recupera — pero el radio de una variable del tutor es mayor que el tutor.
- **Orden al desplegar cambios de variables**: las que un servicio exige para arrancar se ponen **antes** de mergear el código que las exige; las que un guarda prohíbe se quitan **antes** de mergear el código que las prohíbe. Las dos direcciones fallan cerrado, y las dos convierten un merge en un despliegue que no entra.
...
**El repositorio es un workspace pnpm desde PRD-003** (`packages: ["apps/*"]`) y desde **PRD-006 tiene los tres paquetes de ADR-001 § 2**: `apps/web` (Next), `apps/api` (NestJS) y `packages/shared`. La raíz ya **no** es la app: es la raíz del workspace más las herramientas de dato, y sus scripts `build` y `start` sólo delegan a `apps/web` con `pnpm --filter` — eso es lo que hizo que la mudanza no tocara ninguna variable ni ningún Dockerfile de Railway. **`packages/shared` no es un paquete de pnpm**: es una carpeta de fuentes, sin `package.json` y fuera de `packages:`, de modo que el grafo de instalación no cambió. Consecuencias que no son obvias:

- **Ya no hay `tsconfig.json` en la raíz.** Se mudó a `apps/web/`, donde conserva la exclusión de `scripts` y pierde la de `apps`, que dejó de ser descendiente suyo. Efecto colateral aceptado: `drizzle.config.ts` deja de typecheckearse, sin efecto en runtime porque `drizzle-kit` lo empaqueta con esbuild. `apps/web/next.config.mjs` gana además `experimental: { externalDir: true }`, **sin lo cual `next build` no compila nada fuera del directorio de la app** y `packages/shared` no entra en el grafo. Lo que sigue es el porqué histórico de la exclusión, que sigue valiendo: su `include` era `**/*.ts` y se tragaba `apps/api/src`.
- **~~`tsconfig.json` de la raíz excluye `apps`~~ (histórico).** Su `include` es `**/*.ts` y se tragaba `apps/api/src`, así que `next build` empezaba a typecheckear NestJS y fallaba con `TS1206`/`TS1241` sobre los decoradores, que ese tsconfig no habilita. Declarar `packages:` no solo engorda el build del servicio Next: sin el `exclude`, lo rompe.
- **`apps/api` es CommonJS y no declara `"type": "module"`.** Con ESM, `tsc` emite igual (no hay `noEmitOnError`) y el arranque muere con `SyntaxError` sobre el esquema: TypeScript decide el formato por el `package.json` más cercano al **fuente** y Node por el más cercano a la **salida**, y el import cruzado a `packages/shared/src/schema.ts` es donde divergen. Su `tsconfig` lleva `allowImportingTsExtensions` **y** `rewriteRelativeImportExtensions`, **sin** `rootDir`; el entrypoint emitido es `dist/apps/api/src/main.js`, no `dist/main.js`.
...
- **`next-auth` vive en TRES manifiestos** —raíz, `apps/web` y el `catalog:`— y no es redundancia. `packages/shared/src/schema.ts` importa el tipo `AdapterAccountType` de `next-auth/adapters`, y una carpeta de fuentes resuelve sus dependencias caminando hacia arriba desde **su propia** ubicación hasta el `node_modules` de la raíz: no hereda las del paquete que la importó. Sin la línea en el manifiesto de la raíz fallan los builds de los **dos** servicios. Se descubrió ejecutando, no leyendo: PRD-006 § 10 paso 3 no lo listaba. Va con especificador `catalog:` por lo mismo que `drizzle-orm` — declarado en dos sitios con versión literal sería, en palabras del propio `pnpm-workspace.yaml`, "una nota, no un pin".
...
node scripts/load-curriculum.ts          # revisar el diff  (sigue en la raíz)
...
| Date | PRD | Summary |
|---|---|---|
| 2026-07-31 | PRD-006 | **Cierre de ADR-001: los tres paquetes existen.** La app Next pasa de la raíz a `apps/web` y nacen los nueve módulos de `packages/shared`, con lo que se cierran las cinco costuras de re-export que `apps/api` mantenía contra la raíz y los dos duplicados por copia. `packages/shared` **no** es un paquete de pnpm sino una carpeta de fuentes, para no tocar el arreglo de `tsconfig` que PRD-003 dejó como intocable. La raíz conserva `build` y `start` delegando, y por eso el corte **no tocó ninguna variable ni Dockerfile de Railway** — verificado con `nixpacks plan` antes de escribir el commit. El union `TutorEvent`, que ya había derivado (6 miembros en la raíz, 7 en `apps/api`), se parte en `FunnelEvent` y `AuditEvent` y el `track()` de `apps/web` se tipa con el primero, de modo que el evento de auditoría deja de ser emitible desde el proceso que sirve páginas. **Entra la primera CI del repositorio** (`checks.yml`), advisoria, con cinco invariantes por ser público y recibir forks. **Dos cosas aparecieron al ejecutar y no al leer**: `check-curriculum.ts` llevaba roto desde el paso E de PRD-005 —falla igual en `main`, nadie lo vio porque nada lo corría— y `packages/shared` necesita `next-auth` en el manifiesto de la raíz o no compila ninguno de los dos servicios. La re-revisión post-implementación encontró además dos filas del plan de pruebas que el PRD daba por aterrizadas y no lo estaban, y seis controles que pasaban en vacío o se podían saltar. |
| 2026-07-30 | PRD-005 | **El tutor sale del framework web.** El turno entero —prompt certificado, contexto de la lección, ventana, memoria y clave de Anthropic— pasa a `apps/api`; Next queda como proxy que reenvía cookie→Bearer y devuelve el stream sin bufferizar. **El hilo que ve el modelo sale de `conversations`, no del cliente**, que hasta hoy podía fabricar turnos `assistant`. La ruta del navegador se decidió dos veces: la primera versión especificaba el camino directo (CORS + cookie de dominio) y se revirtió al proxy tras encontrar cuatro bloqueantes que existían solo por él y una variante de fijación de sesión que ningún control de servidor cierra. Medido en producción: cabeceras y primer byte juntos a ~3,3 s (el hop queda enterrado bajo la varianza de Anthropic), stream troceado por los dos bordes, y el abandono cancela hasta Anthropic sin persistir. **Corte con incidente**: `apps/api` estuvo caído ~20 min por una referencia cruzada de variables rota al retirar la clave de Next — ver `Entorno y despliegue`. |
