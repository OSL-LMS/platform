...
| Proxy del turno: cookie → Bearer, cuerpo y respuesta sin bufferizar | `POST /api/chat` — `src/app/api/chat/route.ts` + `streamTutorTurn()` en `src/lib/api-client.ts` | PRD-005 |
| Prompt certificado v0.6 | `TUTOR_SYSTEM_PROMPT` — `src/lib/tutor-prompt.ts`, re-exportado por `apps/api/src/tutor/tutor-prompt.ts` | — |
| Bloque de contexto de la lección | `buildLessonContext(moduleLessons, ancestors, lessonSlug?)` — `src/lib/curriculum-context.ts`, re-exportado por `apps/api/src/tutor/` | PRD-002 |
| Memoria de la conversación (escritura) | `getOrCreate`, `append` — `apps/api/src/tutor/conversations.repository.ts` | PRD-005 |
| Memoria de la conversación (lectura para el render) | `loadConversation` — `src/lib/conversations.ts` | PRD-005 |
| Ventana de contexto enviada al modelo | `trimWindow()`, `MAX_WINDOW_MESSAGES = 30` — `src/lib/window.ts`, re-exportado por `apps/api/src/tutor/window.ts` | — |
...
- **La `ANTHROPIC_API_KEY` vive SOLO en el servicio `api`.** El proceso Next se niega a arrancar si la encuentra (`assertNoAnthropicKey` en `src/lib/tutor-turn.ts`, armado en el ámbito del módulo e importado por el handler del proxy). Muerde en `next build`, no solo en arranque, así que la clave presente bloquea el despliegue.
...
- **`TUTOR_VIA_API` sigue puesta en Railway y ningún código la lee.** El paso E retiró la rama. Es inerte, y ahí está el riesgo: alguien en un incidente podría quitarla esperando un rollback y no obtener nada. Borrarla del servicio `tutor-app` es limpieza pendiente.
