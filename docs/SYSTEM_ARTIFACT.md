...
- **El puente entre servicios va por `http://`, no por TLS — aceptado a sabiendas el 2026-07-28.** `API_BASE_URL` apunta a `api.railway.internal:8080`, la red privada de Railway, que aísla pero no cifra. Por ahí viajan el correo del estudiante y **el JWT de sesión**, que es una credencial portadora de ~30 días sin revocación individual. PRD-003 § 8 dice "solo sobre TLS", así que la propiedad **no se cumple**: la sostiene el aislamiento de red, no el cifrado. Esto **no** es un descuido pendiente de arreglar; es una divergencia decidida, y queda aquí porque el PRD está congelado y este es el documento que gana cuando discrepan.

  **Por qué se acepta.** Quien pueda leer ese tráfico ya está dentro de la infraestructura de Railway, entre dos servicios del mismo proyecto. Un atacante en esa posición tiene caminos más directos que esnifar un salto interno — las variables de entorno de los dos servicios, que incluyen `AUTH_SECRET` y `DATABASE_URL`, entre ellos. Cifrar el salto no cambiaría lo que ese atacante puede hacer.

  **Las dos alternativas, y por qué no.** (1) Terminar TLS dentro de `apps/api` obliga a emitir y rotar un certificado para un nombre interno, mantenido por una persona, para defender un tramo que solo Railway ve. (2) Apuntar `API_BASE_URL` a `https://api.contextia.io` daría TLS real cambiando **una variable**, ahora que ese dominio existe con certificado válido — pero saca la llamada a internet y la vuelve a meter, y ocurre en el render de `/chat` y en `/v1/access/trial`, que es justo la ruta donde ADR-001 § 6 vigila un umbral de **+200 ms p95** hasta el primer token. Es la alternativa más barata de las dos y la primera a revisar si alguien mide ese coste y resulta despreciable.

  **Qué invalida esta decisión** (cualquiera de las tres, y entonces se reabre): que el JWT gane alcance o duración —hoy es una sesión de estudiante y nada más—; que aparezca un consumidor no-web y `apps/api` reciba credenciales que no nazcan en Next; o que la red privada de Railway deje de estar acotada al proyecto.
