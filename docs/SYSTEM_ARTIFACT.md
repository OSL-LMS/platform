...
- **El prompt no sabe nada del curso.** Módulo, lecciones y atascos son dato en `curriculum_nodes` y viajan como segundo bloque de system. Cambiar `TUTOR_SYSTEM_PROMPT` exige pasar el banco de 35 evals antes de desplegar — y desde PRD-002 la misma exigencia cubre las llaves de `curriculum/<slug>.json` que alcanzan ese bloque (`stuck`, `outcome`, `audience`, `title`, `scope`), porque la regla se indexa por destino del contenido, no por ruta de archivo.
...
- **El detector de URLs diverge de la letra del PRD, y esa divergencia hay que sostenerla con dos piezas más.** PRD-002 §5.1 lo especifica como "esquema + `:`"; lo enviado exige además un carácter **no blanco** tras los dos puntos, porque la regla literal marca como URL el título real de L5 ("Git: tu trabajo, a salvo y con historia"). Esa exigencia **sola sí relajaba el control**: el parser de URL de la WHATWG elimina tab, LF y CR antes de parsear, así que `https:⟨tab⟩//evil.example.com` no casaba y el navegador navegaba igual — y como el detector es la **única** puerta, ni el control de esquema ni la allowlist de host llegaban a correr. Por eso el valor se normaliza antes de decidir (`stripUrlNoise`) y hay una lista cerrada de esquemas peligrosos (`javascript`, `data`, `vbscript`, `file`, `blob`) que caen aunque lleven espacio detrás. Las seis formas de evasión son casos de prueba en `check-curriculum.ts`. Cualquiera que toque `URL_LIKE` tiene que volver a pensar las tres piezas juntas.
