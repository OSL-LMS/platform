...
Cinco dominios cruzan la frontera. Aquí queda **sólo** la mitad que implementa `apps/web`; la otra está en el documento del paquete que la sirve.
...
- **`temporada`** → `packages/shared/docs/SYSTEM_ARTIFACT.md`. Aquí quedan las funciones puras de `src/lib/schedule.ts` —orden, pausa, formato de fecha y hora, agrupado por temporada— y su render en la home. La tabla `broadcasts`, el archivo autoral y el cargador viven en `shared`. **Tres cosas de esta mitad no son obvias:** el `key` de React y la fila destacada salen del `id` de la emisión y no del `lessonSlug`, porque desde PRD-008 una lección puede emitirse en varias temporadas; la tabla se agrupa sólo con dos o más, así que con una el DOM es idéntico al de antes; y omitir la sección se lleva su entrada de navegación y recalcula la alternancia de bandas, que `page.tsx` declara "en estricto".
...
**Una entrada en `package.json` NO ejecuta nada por sí sola, y las dos convenciones del repositorio son distintas.** Para los scripts de **este** paquete, `.github/workflows/checks.yml` lista un paso explícito por script, así que uno nuevo necesita su entrada `check:*` **y** su paso en el workflow. Pasó una vez con `check:secrets`, que existió sin correr; PRD-007 lo evitó por poco con `check:evidence` porque un revisor lo cazó.

Para los de la **raíz** es al revés: `checks.yml:72` invoca `pnpm curriculum:check`, que sí es un agregado y encadena cinco scripts, así que uno nuevo entra por ahí sin tocar el workflow. PRD-008 §9 corrigió esta entrada, que antes generalizaba la regla de `apps/web` a todo el repositorio y habría llevado a añadir un paso redundante — o, peor, a no engancharlo al agregado.

**Ningún check corre DENTRO del proceso de Next, y eso es un agujero con un incidente detrás.** Los scripts de `scripts/` corren bajo Node pelado, donde `next/cache` ni resuelve; el golden compone datos sin renderizar; y no hay runner de componentes React. Lo que sólo ocurre dentro del servidor de Next —la serialización de `unstable_cache`, el render del JSX— no lo cubre nada. El 2026-08-01 eso costó la landing entera en 500: `getBroadcasts` devolvía `Date` en los tests y cadena en producción. Un check nuevo que dependa de esa frontera necesita verificación manual declarada, no un verde.
...
| Date | PRD | Summary |
|---|---|---|
| 2026-08-01 | PRD-008 | La home lee el calendario de la base en vez de un `const`. La tabla se agrupa por temporada cuando hay más de una —con una, el DOM no cambia— y el copy estático deja de afirmar el tamaño de una temporada. `schedule.ts` conserva sus funciones puras y gana las de composición de texto, para que el golden llame a las mismas que el JSX. |
